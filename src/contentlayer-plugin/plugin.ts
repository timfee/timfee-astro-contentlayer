...
export const runContentlayerDev = async ({ configPath }: PluginOptions) => {
...
export const runContentlayerBuild = async ({ configPath }: PluginOptions) => {
  if (contentlayerInitialized) return
  contentlayerInitialized = true
...
const runMain = core.runMain({
  tracingServiceName: 'next-contentlayer',
  verbose: true, // process.env.CL_DEBUG !== undefined,
})
